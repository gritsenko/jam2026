...
> «Пропустить всё» **реализовано** — круглая кнопка-✕ ([CloseButton.ts](../../src/ui/CloseButton.ts))
> в правом верхнем углу панели разом помечает все оставшиеся уроки уровня виденными
> (тот же `onDone`, что и долистывание до конца). Остальные стретчи §6.1/§8 (отдельные
> PNG-иллюстрации, реплей урока, reduced-motion) — не делались. Поведение зеркалится в
...
- **Admin mode** (`isAdmin()`): `pendingLessons` показывает всё независимо от seen — удобно
  ...
  > **Изменено (отменено):** по запросу баланс-тестирования (Admin почти всегда включён)
  > туториалы больше **не** ре-показываются в Admin. `pendingLessons` теперь чисто по
  > «виденности» (без параметра `admin`); единственный способ ре-армить уроки — СБРОС
  > ПРОГРЕССА (`progress.reset()`). Story-биты (`shouldPlayStory`) Admin по-прежнему
  > переигрывает — изменение касается только обучающих модалок. См.
  > [docs/working/current-state.md](../working/current-state.md).
...
- **Кнопка-✕ «пропустить»** ([CloseButton.ts](../../src/ui/CloseButton.ts)) в правом
  верхнем углу панели — круглая, рисуется кодом (как `GearButton`), помечает все
  оставшиеся уроки уровня виденными и стартует бой (тот же `onDone`). Заголовок
  ужимается чуть сильнее (`titleMaxW`), чтобы длинное имя не залезло под ✕.
...
- **Тело** — абзацы `body[]`, перенос по словам (ширина панели − паддинги). Если в проекте
  ...
  **Размер шрифта авто-подгоняется** под свободное место между иллюстрацией и точками/кнопкой
  (`BODY_FONT_MAX..BODY_FONT_MIN`): короткий урок читается крупно, длинный ужимается, чтобы
  влезть. Сама панель **масштабируется по safe-area** (кап `MAX_CARD_W`/`MAX_CARD_H`), а не
  фиксированного размера — пересобирается в `layout()` при смене габаритов.
...
- **`merge`** — две одинаковые **башни** (`storm_coil`) съезжаются и «схлопываются» в одну
  башню грейдом выше (`storm_coil_g2`) со вспышкой и бейджем `Ур2`. Показываем именно
  башни одного типа (мердж), а не карты — слияние карт это фьюжн (`mech_fusion`).
