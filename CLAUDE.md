...
- **Энергосеть** — перегрузка режет темп каждой башни пропорционально её нагрузке;
  Overdrive (сжигание карты) временно поднимает ёмкость.
- **Экономика** — золото (килл/волна) **и кристаллы**: Perfect Clear даёт кристаллы,
  реролл руки и фьюжн их тратят.
- **Диверсант (Disruptor)** — глушит/станит башни в радиусе.
- **Звёзды за уровень** — на баннере итога 1–3★ по остатку Core (★ пройден,
  ★★ Core ≥ 50%, ★★★ Core не тронут); правило — `BattleSim.starRating`,
  порог — `STAR_TWO_CORE_FRAC` в `combatRules.ts`, отрисовка — `BattleBanner`.
...
**Ещё НЕ реализовано** (проверяй по коду): карты **глобальной модернизации**;
**продажа** башен ([docs/planned/sell-towers.md](docs/planned/sell-towers.md));
**пер-левел волны/сложность** — `levelId` пробрасывается в бой, но не выбирает свой
скрипт волн (все узлы карты играют общий `WAVES`); **дерево технологий, SP-мета и
«Чертежи»** ([docs/planned/progression-and-tech-tree.md](docs/planned/progression-and-tech-tree.md));
3 новых монстра из [docs/planned/enemy-roster-design.md](docs/planned/enemy-roster-design.md)
(в коде 5 врагов + архетип диверсанта); **сохранение звёзд и прогрессия на карте
уровней** — звёзды считаются и показываются в конце боя, но не сохраняются между
сессиями и не открывают узлы (узлы `locked/available/cleared` пока статичны в
`levels.ts`; этим займётся отдельная задача).
