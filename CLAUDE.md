...
направлений нарезаются из ручных **3×3 спрайт-шитов пер-грейд** `<iconKey>_dirs_lvl<n>`
(`lvl1`/`lvl2`/`lvl3` = грейд I/II/III; периметр = стороны света) —
`BattleSim.towerAim` кэширует цель, `SlotView` доворачивается к ней
**неспеша, по одному октанту за `ROT_STEP_SEC` (~0.11с) жёсткой сменой кадра** (без
кроссфейда — бленд двух соседних кадров мигает). **Вращающаяся турель сначала
доворачивается, потом стреляет**: рендер каждый кадр шлёт симу `aimReady`
(`SlotView.isAimed` → `BattleSim.setTowerAimReady`) — пока ствол свингует на лидера,
выстрел придерживается (КД остаётся готовым), потеря DPS почти нулевая (КД мотается во
время поворота). «Доведена» = `displayOctant` сошёлся на `targetOctant` и тот в пределах
одного октанта от `desiredOctant`. **Статичные башни** рапортуют `aimReady = true` и не
гейтятся (у них стрельба чисто по КД/радиусу, `displayOctant` — косметика). Поворот **дебаунсится**
...
гибриды берут шит родителя по `iconKey`. Снаряд и дульная вспышка стартуют из точки
`TOWER_MUZZLE_ANCHORS` ([src/config/cards.ts](src/config/cards.ts)) для текущего октанта,
в двух раскладках: **кончик ствола** (`plasma_shutter`, `railgun`) — пер-октант якорь из
зелёных маркеров аннотированного шита `docs/visual_refs/anchors/<iconKey>_dirs_anchors.png`
(снаряд с кончика вращающегося ствола); **верх-центр спрайта** (`frost_pulse`) — `x=0` по
центру, `y` = верхняя кромка альфы (не ствол; статичная, без `_dirs` — все октанты одинаковы).
`SlotView.muzzleLocal`→`PlatformGrid.muzzleScenePos` кормит точку симу каждый кадр
(`BattleSim.setTowerMuzzle`), фоллбек без рендера — радиальный `TOWER_MUZZLE` (`muzzleLen`).
Пер-грейд `_dirs`-шиты сами читают тир, поэтому **пипсы грейда** у этих башен не рисуются
(флаг `gradeInArt`); у статичных башен без грейд-различимого арта пипсы остаются.
...
- **Боссы уровня** ([docs/done/level-bosses.md](docs/done/level-bosses.md)) — архетип `boss`:
  медленный высоко-HP суперюнит приходит **отдельной финальной волной** (сим-опция `bossId` =
  поле `LevelCombat.bossId`, `totalWaves` = `waves.length+1`), глушит/станит **сразу все** соседние
  башни (усиленный диверсант, та же защита Щита), под него **арена затемняется** (`bossOverlay`),
  токен крупнее. Спрайты = портреты начальников `enemy_boss_*` (ур. 1–7 в `default`).
...
моб-сплиттер, само-реген-щит, «сухой» анти-Wet — **не реализованы**. Legacy-плейсхолдеры
`enemy_boss_warden`/`enemy_boss_titan` так и **не спавнятся** ни в одной волне. **3 моба
поддержки и боссы уровня уже в коде** (см. ниже).
...
- **8-направленные турели**: вращающиеся башни рендерятся из ручных **3×3 спрайт-шитов
  пер-грейд** `<iconKey>_dirs_lvl<n>.png` (`lvl1`/`lvl2`/`lvl3` = грейд I/II/III; прозрачный
  фон; периметр = 8 сторон света от центра). Две раскладки
  ...
  не нужно: положи новые 3×3-шиты как `assets/sprites/<iconKey>_dirs_lvl{1,2,3}.png` и
  перезапусти dev-сервер. **Точка вылета снаряда (дуло)** — пер-октант якоря
  `TOWER_MUZZLE_ANCHORS` в `cards.ts` (доли рендер-текстуры от её центра, порядок октантов
  как у `sliceSheet3x3`). Две раскладки: **кончик ствола** (`plasma_shutter`, `railgun`:
  центроид зелёных маркеров в каждой клетке аннотированного шита
  `docs/visual_refs/anchors/<iconKey>_dirs_anchors.png` → доля клетки) и **верх-центр
  спрайта** (`frost_pulse`: `x=0`, `y` = верх альфа-бокса; статичная — все октанты равны,
  держит `DEFAULT_OCTANT`). Перерисовал шит так, что ствол/силуэт сместился → переснимай
  якоря/`y` (зелёные маркеры или альфа-боксы скриптом на PIL).
  Будущие состояния (idle/attack/loading) — отдельными шитами `<iconKey>_*`. Базовый
  спрайт `<iconKey>.png` — арт для карточки в руке.
...
**6 направленных 3×3-шитов** (`plasma_shutter_dirs_lvl{1,2,3}`/`railgun_dirs_lvl{1,2,3}` —
пер-грейд поворот вращающихся турелей; остальные башни статичны), **8 врагов** (5 базовых + диверсант + 3 моба поддержки
