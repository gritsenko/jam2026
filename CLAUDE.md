...
стороны света; центр не используется) — `BattleSim.towerAim` кэширует цель, `SlotView`
доворачивается к ней **плавно, по одному октанту за ~0.06с через все промежуточные**
(`setAim` ставит цель, `tickAim` шагает). По умолчанию при постройке смотрит на **SE**;
без врага в радиусе **держит последнее направление**. Остальные башни
...
- **8-направленные турели**: вращающиеся башни рендерятся из ручного **3×3 спрайт-шита**
  `<iconKey>_dirs.png` (прозрачный фон; центр = idle, периметр = 8 сторон света от центра).
  Код режет его на лету ([SlotView.sliceSheet3x3](src/ui/SlotView.ts)) — никакого инструмента
...
иконки ресурсов, **6 башен** (ручные спрайты) + **2 направленных 3×3-шита**
(`plasma_shutter_dirs`/`railgun_dirs` — поворот вращающихся турелей; остальные башни
статичны), **5 врагов** (включая диверсанта), узлы карты, **звезда оценки**
