...
«лидеру» (энергоболты, цепь, пробивающий луч). Атакующая турель **доворачивается на
лидера-врага**: 8 направлений выбираются из strip-а `<id>_dirs` поверх статичной базы
(`BattleSim.towerAim` кэширует цель → `SlotView.setAim` снапит ближайший октант;
саппорты не вращаются, гибриды берут strip родителя по `iconKey`; кадры черновые,
дорабатываются). Грейд апает у разных карт разный стат
...
- **8-направленные турели**: атакующие башни рендерятся из strip-а `<iconKey>_dirs`
  (8 равных ячеек N→NW), собранного `tools/pack_dirs.py` из покадровых исходников в
  `tools/dir_frames/`. Доработать поворот: пере-/догенерировать кадры → `pack_dirs.py`
...
На диске: все фоны (дизельпанк/пустыня), платформа-плита, логотип, аватар + рамка,
иконки ресурсов, **6 башен** (4 атакующие турели + 2 саппорта) и **4 strip-а направлений**
(`plasma_shutter_dirs`/`frost_pulse_dirs`/`storm_coil_dirs`/`railgun_dirs` — по 8 кадров
поворота турели), **5 врагов** (включая диверсанта), узлы карты, **звезда оценки**
(`icon_star`), **3 карты модернизации** (`isolation_circuit`/`elemental_focus`/
`emergency_overdrive`) и **5 символов стихий** (`sym_*`, см.
[docs/planned/tower-readability.md](docs/planned/tower-readability.md) §3). HUD-чром
(панели/кнопки/бейджи/плита) рисуется **процедурно** (`drawPanel`/`PlatformGrid.buildPlate`
— тёмный металл + заклёпки + фаска); legacy-спрайты `ui_panel`/`ui_button`/`ui_card_frame`
в коде не подключены. Плейсхолдерами остаются `decor_pylon` (нигде не подключён) и
