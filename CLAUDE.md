...
[src/config/cards.ts](src/config/cards.ts)): **композитный** (пока `plasma_shutter`) —
центр = **неподвижная база** (рисуется один раз под головой), периметр = **только
поворотная голова** (один спрайт, меняет кадр); **старый** (`railgun`) — каждая ячейка
целая турель (центр = idle), крутится весь спрайт. Остальные башни
...
- **8-направленные турели**: вращающиеся башни рендерятся из ручного **3×3 спрайт-шита**
  ...
  (`plasma_shutter`) — **центр = неподвижная база** (рисуется один раз), периметр = **только
  поворотная голова** (крутится; база на месте); **старая** (`railgun`) — каждая ячейка
