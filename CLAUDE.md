...
гибриды берут шит родителя по `iconKey`. Снаряд и дульная вспышка вылетают из **точного
кончика ствола текущего кадра поворота**: пер-октант якоря дула `TOWER_MUZZLE_ANCHORS`
([src/config/cards.ts](src/config/cards.ts)) измерены из аннотированных шитов
`docs/visual_refs/anchors/<iconKey>_dirs_anchors.png` (зелёные маркеры);
...
- **8-направленные турели**: вращающиеся башни рендерятся из ручных **3×3 спрайт-шитов
  ...
  перезапусти dev-сервер. **Точка вылета снаряда (дуло)** — пер-октант якоря из
  аннотированного шита `docs/visual_refs/anchors/<iconKey>_dirs_anchors.png` (зелёные
  маркеры на кончиках стволов) → `TOWER_MUZZLE_ANCHORS` в `cards.ts` (доли клетки,
  порядок октантов как у `sliceSheet3x3`). Перерисовал стволы → переснимай якоря.
