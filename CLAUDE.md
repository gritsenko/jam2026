...
в [docs/working/cards.md](docs/working/cards.md) и [docs/cards.json](docs/cards.json). Полное ТЗ
каркаса — [BRIEF.md](BRIEF.md). Визуальный референс — [docs/style_ref.png](docs/style_ref.png).

## Текущее состояние (ВАЖНО)

Базовый бой реализован поверх каркаса. Работает headless-симуляция в
[src/game/BattleSim.ts](src/game/BattleSim.ts) (без Pixi), которую сцена только
рендерит: **волны врагов** (скрипт волн — [src/config/waves.ts](src/config/waves.ts)),
враги **выходят из-за нижнего края экрана** и идут по кольцевой дороге
([src/game/path.ts](src/game/path.ts), путь — `ENEMY_PATH` в
[src/config/combatRules.ts](src/config/combatRules.ts)); **атака башен** — у каждой
атакующей карты свой **радиус в клетках** (`rangeCells`), бьёт только ближнюю дорогу,
по «лидеру», энергоболтами. Грейд апает у разных карт разный стат (`upgrade`:
`power`/`tempo`/`range` — урон/темп/радиус, см. `towerStats`). Радиус показывается
эллипсом при перетаскивании карты до установки. Есть HP/смерть/награда (только
**золото**), утечка → урон **Core Integrity**, победа/поражение с баннером.
Перегрузка энергии замедляет темп всех башен, Overdrive (сжигание) временно поднимает
ёмкость. Рука по умолчанию **3 слота** (расширяемо позже). Тюнинги — в `combatRules.ts`,
`enemies.ts`, `cards.ts`.

**Очков синергии (SP) в игре больше нет** — установка карт ничем не гейтится, кроме
свободного слота (реальная «цена» — нагрузка на энергосеть). 

**Ещё НЕ реализовано** (проверяй по коду, не по этому списку): таблица стихийных
**синергий**/реакций (Паровой Выброс и т.п.), **мердж** карт в грейды, эффекты
карт-поддержки (барьер Щита, замедление Стабилизатора), карты глобальной
модернизации, трата золота/кристаллов (экономика — только награды за килл/волну).
Стартовая раскладка платформы и ресурсы по-прежнему берутся из мока
[src/config/battleState.ts](src/config/battleState.ts).
...
- `docs/working/` — **рабочие**: зеркало текущего кода (`current-state.md`,
  `enemy-balance.md`, `cards.md`). Описывают «как есть сейчас».
...
src/
  ...
  game/                 # ЛОГИКА боя без Pixi: BattleSim (симуляция), path (ArenaPath, кольцо)
  ...
  config/               # ДАННЫЕ отдельно от логики: types, cards, enemies, levels, waves,
                        #   battleRules, combatRules, battleState (мок), assetManifest
